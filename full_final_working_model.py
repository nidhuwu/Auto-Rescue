...
def get_nearest_user(accident_lat, accident_lon):
    ...
    for user in users_data:
        ...
        if distance < min_distance:
            ...
            nearest_user = {'id': user_id, 'name': user_name, 'email': user[2], 'distance': distance}

    return nearest_user

def send_email_with_attachment(image_path, nearest_user):
    ...
    body = f'Car accident detected near location coordinates f"Latitude: {latitude}, Longitude: {longitude}". Please check the attached image for details.'

    message = MIMEMultipart()
    message['From'] = sender_email
    message['To'] = nearest_user['email']
    ...
    try:
        with smtplib.SMTP(smtp_server, smtp_port) as server:
            ...
            server.sendmail(sender_email, nearest_user['email'], message.as_string())
        print(f'Mail with image sent successfully to {nearest_user["name"]} ({nearest_user["email"]})!')
...
def predict_and_send_email(img_path):
    ...
    if prediction_value > 0.65:
        ...
        nearest_user = get_nearest_user(latitude, longitude)

        if nearest_user:
            send_email_with_attachment(img_path, nearest_user)
        else:
